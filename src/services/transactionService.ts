...
function docToTransaction(doc: DocumentData): Transaction {
  ...
  return {
    ...
    tags: data.tags || [],
    timestamp: data.timestamp?.toDate ? data.timestamp.toDate() : (data.timestamp?._seconds ? new Date(data.timestamp._seconds * 1000) : new Date(data.timestamp || Date.now())),
    ...
    updatedAt: data.updatedAt?.toDate ? data.updatedAt.toDate() : (data.updatedAt?._seconds ? new Date(data.updatedAt._seconds * 1000) : null),
  };
}
